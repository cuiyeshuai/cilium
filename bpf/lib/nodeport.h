						  ...
						  __s8 *ext_err)
{
...
#ifdef ENABLE_CRAB
	{
		...
		if (redir_opt->index == 0) { // client egress, put masquerade ip to ip3
		    ...
		    // 	(bpf_ntohs(0) << 16) | bpf_ntohs(0));
			
			// set src ip to client pod ip
			// if (!revalidate_data(ctx, &data, &data_end, &ip4))
			// 	return DROP_INVALID;
			...
			// cilium_dbg3(ctx, DBG_CT_LOOKUP4_1, redir_opt->ip1, redir_opt->ip3,
		    // 	(bpf_ntohs(redir_opt->port2) << 16) | bpf_ntohs(redir_opt->port1));
			//rewrite src port
		}
		// else if (redir_opt->index == 1) { //LB egress rewrite src addr to be client masqurade addr
		// 	__be32 client_masq_ip = redir_opt->ip3;
		// 	__be16 client_masq_port = redir_opt->port1;
		// 	int diff;
			
		// 	cilium_dbg3(ctx, 0, 197, 197, 197);
		// 	//rewrite src addr
		// 	if (!revalidate_data(ctx, &data, &data_end, &ip4))
		// 		return DROP_INVALID;
		// 	ret = ctx_store_bytes(ctx, ETH_HLEN + offsetof(struct iphdr, saddr), &client_masq_ip, 4, 0);
		// 	if (ret < 0) return DROP_WRITE_ERROR;
		// 	//rewrite src port
		// 	if (!revalidate_data(ctx, &data, &data_end, &ip4))
		// 		return DROP_INVALID;
		// 	ret = ctx_store_bytes(ctx, ETH_HLEN + ipv4_hdrlen(ip4) + offsetof(struct tcphdr, source), &client_masq_port, 2, 0);
		// 	if (ret < 0) return DROP_WRITE_ERROR;
		// 	cilium_dbg3(ctx, 0,50,50,50);
		// 	if (!revalidate_data(ctx, &data, &data_end, &ip4))
		// 		return DROP_INVALID;
		...
		// 	if (ipv4_csum_update_by_diff(ctx, ETH_HLEN, diff) < 0)
		// 		return DROP_CSUM_L3;
		// 	cilium_dbg3(ctx, 0, 123, 123, 123);
		// }
		// cilium_dbg3(ctx, 0, 122, 122, 122);
...
int tail_nodeport_nat_egress_ipv4(struct __ctx_buff *ctx)
{
	...
	cilium_dbg3(ctx, 0, crab_parse_ret, crab_parse_ret, 0); // very important line!!!!!!!!!! error if not here
	if (crab_parse_ret == 1) { // Found option, LB egress, nothing special to do
		redir_opt = (struct redir_opt_complete *)parser.cur_pos;
		if ((void*)redir_opt + sizeof(struct redir_opt_complete) > data_end)
			return DROP_INVALID;
	} else goto skip_crab_fib;
	if (redir_opt->index == 1) {
		__be32 client_masq_ip = redir_opt->ip3;
		target.addr = client_masq_ip;
	} else if (redir_opt->index == 5) {
		__be32 client_masq_ip = redir_opt->ip2;
		target.addr = client_masq_ip;
	}
					...
					__s8 *ext_err)
{
	...
	// check index
	if (redir_opt->index == 0 || redir_opt->index == 4) { // LB ingress, redirect to service
		...
		// __be32 backend_ip = redir_opt->ip3;
		// __be16 backend_port = redir_opt->port1;
		int diff;
		int index_store = redir_opt->index;
		...
		ret = ctx_store_bytes(ctx, l3_off + offsetof(struct iphdr, daddr),
				&new_daddr, 4, 0);
		// else {
		// 	__be32 current_node_ip = IPV4_DIRECT_ROUTING;
		// 	ret = ctx_store_bytes(ctx, l3_off + offsetof(struct iphdr, daddr),
		// 			&current_node_ip, 4, 0);
		// }
		// Modify src ip
		...
		index = old_index + 1;
		ret = ctx_store_bytes(ctx, l3_off + sizeof(struct iphdr) + sizeof(struct tcphdr) + offsetof(struct redir_opt_complete, index),
			      &index, 1, 0);
		if (ret < 0) return DROP_WRITE_ERROR;
		if (!revalidate_data(ctx, &data, &data_end, &ip4))
			return DROP_INVALID;
		...
		struct crab4_pair_long crab_value = {};
		struct crab4_pair_long *temp = NULL;
		// __be32 sum_diff;
		// struct csum_offset csum_off = {};
		l4_off = l3_off + ipv4_hdrlen(ip4);

		// store service_ip & service_port in crab ebpf map
		
		if (redir_opt->index == 5) {
			tcph = (struct tcphdr *)((void*)ip4 + ipv4_hdrlen(ip4));
			if ((void*)tcph + sizeof(struct tcphdr) + sizeof(struct redir_opt_complete) > data_end)
				return DROP_INVALID;
			client_port = tcph->source;
		}
		crab_key.addr = ip4->saddr;
		crab_key.port = tcph->source;
		crab_value.addr1 = client_ip;
		crab_value.port1 = client_port;
		crab_value.addr2 = service_ip;
		crab_value.port2 = service_port;
		ret = map_update_elem(&LB4_CRAB_MAP_LONG, &crab_key, &crab_value, BPF_ANY);
		if (ret < 0) return DROP_WRITE_ERROR;
		// read the value from map
		temp = map_lookup_elem(&LB4_CRAB_MAP_LONG, &crab_key);
		if (temp == NULL) return DROP_INVALID_TC_BUFFER;
		// cilium_dbg3(ctx,DBG_CT_LOOKUP4_1, temp->addr1 , temp->addr2, ((bpf_ntohs(temp->port1) << 16) | bpf_ntohs(temp->port2)));

		// write client ip & port to packet
		// old_ip = ip4->saddr;
		// old_port = tcph->source;
		// sum_diff = csum_diff(&old_ip, 4, &client_ip, 4, 0);
		// if (ipv4_csum_update_by_diff(ctx, l3_off, sum_diff) < 0)
		// 	return DROP_CSUM_L3;
		...
		// if (has_l4_header)
		// 	csum_l4_offset_and_flags(IPPROTO_TCP, &csum_off);
		// if (csum_l4_replace(ctx, l4_off, &csum_off, old_port, client_port, sizeof(client_port)) < 0)
		// 	return DROP_CSUM_L4;
		...
		// 	(bpf_ntohs(service_port) << 16) | bpf_ntohs(client_port));
		// cilium_dbg3(ctx, DBG_CT_LOOKUP4_1, client_ip, redir_opt->ip3,
		// 	(bpf_ntohs(service_port) << 16) | bpf_ntohs(client_port));
		// Service conntrack
		tcph = (struct tcphdr *)((void*)ip4 + ipv4_hdrlen(ip4));
		if ((void*)tcph + sizeof(struct tcphdr) > data_end)
		...
		crab_tuple.dport = client_port; // Extra care, Cilium handle tuple port differently
		crab_tuple.sport = service_port;
		crab_tuple.flags = TUPLE_F_SERVICE;
		lb4_fill_key(&crab_key, &crab_tuple);
		// cilium_dbg3(ctx, DBG_CT_LOOKUP4_1, crab_key.address, crab_key.address,
		// 		(bpf_ntohs(crab_key.dport) << 16) | bpf_ntohs(crab_key.dport));
		...
		for (slot = 1; slot <= 2; slot++) { // loop through all the backends to see if there is a match, verifier complains if number is too large
			...
			if (be != NULL) {
				...
				// cilium_dbg3(ctx, DBG_CT_LOOKUP4_1, backend->address, backend_ip,
				// 	(bpf_ntohs(backend_port) << 16) | bpf_ntohs(backend->port));
			}
		}
...
skip_crab:
	...
	if (svc) {
		...
		if (lb4_to_lb6_service(svc)) {
...
#if defined(ENABLE_CRAB) // n-s nodeport ingress
			union tcp_flags tcp_flags = { .value = 0 };
			bool is_tcp_crab;
			bool is_syn_crab = false;
			bool is_ack_crab = false;
			__be32 nodeport_ip;
			__be16 nodeport_port;
			// __be32 backend_ip;
			// __be16 backend_port;
			...
			// // resolve service backend and put it in option, hard to mimic nodeport behavior as we don't have the node ip
			// // {
			// // 	__u32 backend_id;
			// // 	struct lb4_backend *backend;
			...
			// // 	cilium_dbg3(ctx, DBG_CT_LOOKUP4_1, backend_ip, backend_ip,
			// // 		(bpf_ntohs(backend_port) << 16) | bpf_ntohs(backend_port));
			// // }
			nodeport_ip = ip4->daddr;
			nodeport_port = tcph->dest;

			// check dest IP is current node IP
			if (nodeport_ip != IPV4_DIRECT_ROUTING || after_lb) {
				goto skip_ns_option;
			}

			// send_trace_notify(ctx, TRACE_FROM_LXC, SECLABEL, 0, 0, 0,
			// 		TRACE_REASON_UNKNOWN, TRACE_PAYLOAD_LEN);
			// cilium_dbg3(ctx, 0, 200, 200, 0);
			...
			if (is_syn_crab && not_critical_services(&tuple) && !is_ack_crab) {
				...
				option_value.ip1 = tuple.saddr;
				option_value.ip2 = nodeport_ip;
				option_value.ip3 = nodeport_ip;
				option_value.port1 = tcph->source;
				option_value.port2 = nodeport_port;
				option_value.index = (__u8)4;
				option_value.padding = (__u8)0;

				redir_opt_new = &option_value;
				// send_trace_notify(ctx, TRACE_FROM_LXC, SECLABEL, 0, 0, 0,
				// 	TRACE_REASON_UNKNOWN, TRACE_PAYLOAD_LEN);
				...
				// 	(bpf_ntohs(option_value.port1) << 16) | bpf_ntohs(option_value.port1));
				
				if (!revalidate_data(ctx, &data, &data_end, &ip4)) 
					return DROP_INVALID;
				if (crab_add_tcp_option(ctx, ip4->tot_len, ip4, tcph, redir_opt_new, REDIR_OPT_COMPLETE) < 0) {
					return DROP_INVALID;
				}
...
crab_handle:
	 ...
	 */
	// cilium_dbg3(ctx, 0, 122, backend_local == 1, 0);
	if (backend_local || !nodeport_uses_dsr4(&tuple)) {
		struct ct_state ct_state = {};

		// cilium_dbg3(ctx, 0, 124, 124, 0);
		ret = ct_lb_lookup4(get_ct_map4(&tuple), &tuple, ctx, l4_off,
				    has_l4_header, CT_EGRESS, &ct_state, &monitor);
		switch (ret) {
		case CT_REPLY:
